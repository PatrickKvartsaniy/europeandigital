...
@section('content')
            ...
            <form action="{{urlloc('/admin/blog/article/edit')}}" accept-charset="UTF-8"
                ...
                <section>
                    ...
                    <div class="uploadPhoto">
                        @if(isset($post->files->where('is_main', '=', true)->first()->full_path))
                            <div class="text-center block-avatar block-empty-avatar">
                                <img src="/files{{$post->files->where('is_main', '=', true)->first()->full_path}}">
                            </div>
                            <div class="text-center block-avatar block-empty-avatar">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="post-category-1">
                                <li class="header">
                                    ...
                                    <div class="post-add-label relative inline_block">
                                        ...
                                        <button type="button" onclick="add_serie(this);" data-post-id="{{$post->id}}" class="addLabel"><i class="fa fa-plus-circle" aria-hidden="true"></i>
                                        </button>
                                    </div>
                                </li>
                                <section id="serie_holder_block">
                                    @if(isset($series) && !empty($series))
                                        @foreach($series as $serie)
                                            <li>
                                                <input name="serie_id_post" value="{{$serie->id}}" type="radio" @if($serie->id == $post->serie_id) checked
                        ...
                        @if(isset($post->tags) && !empty($post->tags))
                            @foreach($post->tags as $tag)
                                <span class="output-label">
                                    <i class="fa fa-times" onclick="delete_post_tag(this);" data-postid="@if(isset($post['id']) && !empty($post['id']) ){{ $post['id'] }}@endif" data-tagid="{{$tag->id}}" aria-hidden="true"></i>{{$tag->name}}
                                </span>
                            @endforeach
                        @endif
                    </div>
                ...
                <input name="serie_id" hidden
                       value="@if(isset($post['id']) && !empty($post['id']) ){{ $post['id'] }}@endif" class="hidden">
            </form>
        </div>
    ...
    <script>
        ...
        function delete_post_tag(_this) {
            $.ajax({
                method: "POST",
                url: '{{urlloc('/admin/blog/article/delete-tag')}}',
                dataType: 'json',
                data: {
                    _token: '{{csrf_token()}}',
                ...
                $(_this).closest('span.output-label').remove();
            });
        }
