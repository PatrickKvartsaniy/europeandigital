...
Class PostController extends Controller
{
     ...
     * @throws \Throwable
     */
    public function showPost(Request $request, $articleSlug)
    {
        $filterByDaysAgoData = []; // getFilteringPeriodsInDays();

        $article = Post::getBySlug($articleSlug);
        ...
        $popularPosts = Post::getPopularLimit([$article->id]);
        // TODO!!!!!!!
        $relatedPost = Post::getRelatedLimit([$article->id]);
        $recentPosts = Post::getRecentLimit([$article->id]);
        $previousPost = Post::getPreviousById($article->id);
        $nextPost = Post::getNextById($article->id);
        ...
        return view('site.blog.post.show_post', $variables)->render();
    }
    ...
    public function editPost(Request $request)
    {
        ...
        $post->main_img = $request->input('main_img');
